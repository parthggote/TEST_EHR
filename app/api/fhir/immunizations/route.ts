/**
 * FHIR Immunizations API Route
 * Handles fetching immunizations for a specific patient
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const bundle = await epicClient.getPatientImmunizations(accessToken, patientId);

    return NextResponse.json(bundle);
  } catch (error) {
    console.error('Immunizations API error:', error);
    ...
    return NextResponse.json(
      {
        error: 'Failed to fetch immunization data',
