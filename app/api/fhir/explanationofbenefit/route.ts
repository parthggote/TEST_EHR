/**
 * FHIR ExplanationOfBenefit API Route
 * Handles fetching explanation of benefit for a specific patient
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const bundle = await epicClient.getPatientExplanationOfBenefit(accessToken, patientId);

    return NextResponse.json(bundle);
  } catch (error) {
    console.error('ExplanationOfBenefit API error:', error);
    ...
    return NextResponse.json(
      {
        error: 'Failed to fetch explanation of benefit data',
