/**
 * FHIR DocumentReferences API Route
 * Handles fetching document references for a specific patient
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const bundle = await epicClient.getPatientDocumentReferences(accessToken, patientId);

    return NextResponse.json(bundle);
  } catch (error) {
    console.error('DocumentReferences API error:', error);
    ...
    return NextResponse.json(
      {
        error: 'Failed to fetch document reference data',
